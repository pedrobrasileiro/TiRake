...
def android_to_ubertesters file, count, notes
  copy_to_ubertesters file, count, "android", notes
end
...
def copy_to_ubertesters file, count, os, notes
  if os.nil? || os.empty?
    puts "OS name required."
    return nil
  end
  ...
  if (!File.exists?(file))
    ...
    sleep 5
    count += 1
    copy_to_ubertesters file, count, os, notes
  elsif File.mtime(file) <= Time.now-60*60 # 1 hours
    puts "File #{file} is very old or iTunes do not copy this file. Trying again in 5 seconds..."
    sleep 5
    count += 1
    ...
    puts "Sending #{tf_file} to Ubertesters with notes '#{notes}'"
    `curl -X POST -H 'X-UbertestersApiKey:#{$uber_api_token}' -F file='@#{tf_file}' -F notes='#{notes}' -F status='in_progress' --progress-bar -o tirake_ubertesters_upload.log http://beta.ubertesters.com/api/client/upload_build.json`
